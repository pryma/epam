...
public class StaxParseCommand implements ICommand{
	...
	public String execute(HttpServletRequest request) throws CommandException {
		try {
			List<DepositModel> deposits = ParserService.parse(DaoType.STAX, request.getParameter(RequestParameter.XML_FILE_NAME.getParameter()));
			request.setAttribute(RequestParameter.DEPOSIT_LIST.getParameter(), deposits);
		} catch (ServiceException e) {
			throw new CommandException("Cannot execute StaxParseCommand", e);
		}
