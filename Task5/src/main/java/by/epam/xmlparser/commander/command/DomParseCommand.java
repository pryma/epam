...
public class DomParseCommand implements ICommand {

	public String execute(HttpServletRequest request) throws CommandException {
		
		try {
			List<DepositModel> deposits = ParserService.parse(DaoType.DOM, request.getParameter(RequestParameter.XML_FILE_NAME.getParameter()));
			request.setAttribute(RequestParameter.DEPOSIT_LIST.getParameter(), deposits);
		} catch (ServiceException e) {
			throw new CommandException("Cannot execute DomParseCommand", e);
		}
